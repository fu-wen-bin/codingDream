...
function Weather () {
  ...
  useEffect(() => {
    window._AMapSecurityConfig = {
      securityJsCode: '7fd19de1efdc68fd0c16f0b0e6045ce8',
    }
    AMapLoader.load({
      key: '4b0c3ee2494b2e4320cccf8c95725c7e', // 申请好的Web端开发者Key，首次调用 load 时必填
      ...
      <div className="hd">
        ...
        <div className="changeCity">
          <div className="changeCity-btn">
            <i className="iconfont icon-24gf-city"></i>
            切换城市
          </div>
        </div>
      </div>
              ...
              futureWeather.map(item => {
                return (
                  <li className="future-item" key={item.date}>
                    ...
                    <span className="max">{item.dayTemp}℃ </span>
                    <span className="min">/ {item.nightTemp}℃</span>
